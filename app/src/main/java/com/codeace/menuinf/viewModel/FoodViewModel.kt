...
class FoodViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun searchItem(text: String): List<FoodData> {
        return foodRepository.getMenu().filter { data ->
        ...
        if (!selectedCategories.isNullOrEmpty()) {
            selectedCategories.forEach {
                foodRepository.getMenu()
                    .filter { s -> s.food_category == categoryListItems.toList()[it] && s.food_price in minPrice..maxPrice }
                    .forEach {
                        listItem.add(it)
                    }
            }
    ...
    fun delete(foodData: FoodData, pos: Int) {
        foodRepository.delete(foodData, pos)
        isChanged = true
    }
